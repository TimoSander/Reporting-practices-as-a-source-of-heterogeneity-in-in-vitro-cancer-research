##### R Code for "Reporting practices as a source of heterogeneity in in vitro cancer research" ######
...
# Preliminary exclusions (reasons can be found in the file "Later exclusions from SR & MA"):
...
AR_updated$combined_U87_source <- ifelse(AR_updated$What.other.source.for.U87.cells.did.they.have. == "Dr. Adriana da Silva Santos Duarte from Hemocenter, State University of Campinas, Campinas, SÃÂ£o Paulo, Brazil" |
...
AR_updated %>% distinct(StudyIdStr, .keep_all = T) %>% select_(63) %>% table()
...
AR_updated %>% distinct(StudyIdStr, .keep_all = T) %>% select_(64) %>% table()
  ...
  ifelse(
    ...
    ifelse(
      AR_updated$Which.other.outcome.measure.method.is.used. == "WST-1 (4-[3-(4- iodophenyl)-2-(4-nitrophenyl)-2H-5-tetrazolio]-1,3-benzene disulphonate) colorimetric assay" |
        ...
        AR_updated$Which.other.outcome.measure.method.is.used. == "water soluble tetrazolium salts 1 (WSTâ€‘1) assay", "WST-1 assay, colorimetric",
      "other")
  )
)
...
List18 <- list()
for (i in seq(1, nrow(ARSC), 1)) {
  dummy18 <- AR_updated %>% filter(StudyIdStr == ARSC[i,1]) %>% select_(65) %>% unique() %>% nrow() 
  List18[[length(List18)+1]] = dummy18
}
...
AR_updated %>% filter(is.element(StudyIdStr, Studies_with_one_assay$ARSC.StudyIdStr)) %>% distinct(StudyIdStr, .keep_all = T) %>% select_(65) %>% table()
# CAVE: Some articles use multiple outcome assessment methods, these are checked manually in the csv file below.

# Command for exporting it as csv:
# AR_updated %>% filter(is.element(StudyIdStr, Studies_with_one_assay$ARSC.StudyIdStr)) %>% distinct(StudyIdStr, .keep_all = T) %>% select_(65) %>% table() %>% write.csv("assay_frequency.csv", row.names = F)
...
ARSCF1R <- AR_updated[, c(62,59,47,45,41,1,66)] %>% filter(Control == FALSE) %>% distinct(joined_StudyID_Control,.keep_all = T)
...
ARC1R <- AR_updated %>% distinct(joined_StudyID_Control,.keep_all = TRUE) %>% select_(1,67,59,57) %>% filter(Control == TRUE)
...
AR1Rcellconc <- AR_updated %>% distinct(StudyIdStr, .keep_all = TRUE) %>% select_(1,62)
...
AR_updated %>% distinct(StudyIdStr, .keep_all = TRUE) %>% select_(62) %>% table()

# Exporting as csv:
# AR_updated %>% distinct(StudyIdStr, .keep_all = TRUE) %>% select_(62) %>% table() %>% write.csv("Comb_cell_concentrations.csv", row.names = F)
...
ARIP1R[ARIP1R$StudyIdStr == "fa8c41e6-ab4d-4e97-a846-75208625a8cc", 27] <- "not reported"
...
bpara[1:10,2] <- c("Mycoplasma exclusion", "U-87 MG age", "Source of U-87 MG", "Cell passaging criteria", "Antibiotics", "FBS source", "U87 authentification", "Cell growth assay", "Glucose level","Source of TMZ")
...
## Adding a column with the raw mean difference D:
AR_OD_MA_T_f_RMD$D <- ifelse(AR_OD_MA_T_f_RMD$Greater.is.worse. == TRUE,
                             (AR_OD_MA_T_f_RMD$Average.1 - AR_OD_MA_T_f_RMD$Average) / AR_OD_MA_T_f_RMD$Average.1 * 100,
                             (AR_OD_MA_T_f_RMD$Average - AR_OD_MA_T_f_RMD$Average.1 / 100 * 100))



### Variance of the effect size D

# Calculating the pooled SD (with the SDs divided by the viability of control and multiplied with 100):
AR_OD_MA_T_f_RMD$S_pooled_relativepercent <- ifelse(AR_OD_MA_T_f_RMD$Greater.is.worse. == TRUE, 
                                                    sqrt(
                                                      (((AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort.1 - 1) * (AR_OD_MA_T_f_RMD$SD.1 / AR_OD_MA_T_f_RMD$Average.1 * 100)^2) +
                                                         ((AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort - 1) * (AR_OD_MA_T_f_RMD$SD / AR_OD_MA_T_f_RMD$Average.1 * 100)^2)) /
                                                        (AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort.1 + AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort - 2)),
                                                    sqrt(
                                                      (((AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort.1 - 1) * (AR_OD_MA_T_f_RMD$SD.1 / 100 * 100)^2) +
                                                         ((AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort - 1) * (AR_OD_MA_T_f_RMD$SD / 100 * 100)^2)) /
                                                        (AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort.1 + AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort - 2)))

# Are there rows where the number of experiments is one, so the pooled SD becomes NaN?
AR_OD_MA_T_f_RMD %>% filter(is.na(S_pooled_relativepercent))
# This is one row where number of experiments is one in control and tmz group, so the denominator of the pooled SD becomes zero and therefore NaN!

# For this data row we assume the mean SD of tmz and control data as the pooled SD:
AR_OD_MA_T_f_RMD$S_pooled_relativepercent[AR_OD_MA_T_f_RMD$StudyIdStr == "d35a2ba9-5cdd-4314-aa6c-b2e3b01d3f6b"] <- 9.25

# Adding a column with the variance of the raw mean difference:
AR_OD_MA_T_f_RMD$Variance_of_D <- ((AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort.1 + AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort) /
                                     (AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort.1 * AR_OD_MA_T_f_RMD$Number.of.animals.in.cohort)) * AR_OD_MA_T_f_RMD$S_pooled_relativepercent



### Excluding data points with treatment time = 0:

AR_OD_MA_T_f_RMD <- AR_OD_MA_T_f_RMD %>% filter(Time != 0)
...
### Moderators from AR:
AR_OD_MA_T_f_RMD$authentification <- AR_updated[AR_OD_MA_T_f_RMD$rn, 19]
AR_OD_MA_T_f_RMD$mycoplasma <- AR_updated[AR_OD_MA_T_f_RMD$rn, 20]
AR_OD_MA_T_f_RMD$glucose <- AR_updated[AR_OD_MA_T_f_RMD$rn, 38]
...
AR_OD_MA_T_f_RMD$antibiotics <- AR_updated[AR_OD_MA_T_f_RMD$rn,36]
...
# Adding it to AR_OD_MA_T_f_RMD
AR_OD_MA_T_f_RMD$U87_age_passages <- AR_updated[AR_OD_MA_T_f_RMD$rn, 18]

# replace empty cells with NA:
AR_OD_MA_T_f_RMD$U87_age_passages[AR_OD_MA_T_f_RMD$U87_age_passages == ""] <- NA

# Assuming the center of a range if the age was reported as range:
AR_OD_MA_T_f_RMD$U87_age_passages[AR_OD_MA_T_f_RMD$joined_StudyID_EL == "65e0c64b-fd79-4f96-b170-ef785d6aa038 e"] <- 10

# converting the age into numeric values:
AR_OD_MA_T_f_RMD$U87_age_passages <- as.numeric(AR_OD_MA_T_f_RMD$U87_age_passages)
...
AR_OD_MA_T_f_RMD %>% filter(is.element(StudyIdStr, ARSCT1R$StudyIdStr)) %>% nrow() == nrow(AR_OD_MA_T_f_RMD)

# Which studies have no control parameters?:
AR_OD_MA_T_f_RMD %>% filter(!is.element(StudyIdStr, ARSCT1R$StudyIdStr)) %>% select_(1) %>% unique()
...
# Adding the type of control using the ARSCT1R dataframe:
for (ctparameter in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter, 29] <- ARSCT1R %>% filter(StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter, 1]) %>% select_(8) 
}
...
# adding it to AR_OD_MA_T_f_RMD:
AR_OD_MA_T_f_RMD$cell_concentration <- AR_updated[AR_OD_MA_T_f_RMD$rn, 62]

# converting it to numerical values:
AR_OD_MA_T_f_RMD$cell_concentration <- as.numeric(AR_OD_MA_T_f_RMD$cell_concentration)



### Decadic logarithm of the TMZ conc.:
AR_OD_MA_T_f_RMD$tmz_conc_microM_log10 <- log(AR_OD_MA_T_f_RMD$tmz_concentration_in_microM_numeric, 10)


## Decadic logarithm of the Time (treatment duration) to AR_OD_MA_T_f_RMD:
AR_OD_MA_T_f_RMD$time_log10 <- log(AR_OD_MA_T_f_RMD$Time, 10)
...
library(drc)
model_hill <- drm(D ~ tmz_concentration_in_microM_numeric, fct = LL.4(), data = AR_OD_MA_T_f_RMD)
summary(model_hill)
# Some coefficients are not significant (p > 0). However, we use this model as an possible moderator of the effect to test its appropriateness.

# adding a column with the 4-parameter-log-logistic tmz concentration predictor:
AR_OD_MA_T_f_RMD$loglog4pred <- model_hill$coefficients[3] + ((model_hill$coefficients[2] - model_hill$coefficients[3]) / (1 + (AR_OD_MA_T_f_RMD$tmz_concentration_in_microM_numeric / model_hill$coefficients[4])^model_hill$coefficients[1]))
...
# building a 4-parameter hill-equation with the help of the drc package:
model_hill_time <- drm(D ~ Time, fct = LL.4(), data = AR_OD_MA_T_f_RMD)
summary(model_hill_time)

# adding a column with the 4-parameter-log-logistic time predictor:
AR_OD_MA_T_f_RMD$loglog4predtime <- model_hill_time$coefficients[3] + ((model_hill_time$coefficients[2] - model_hill_time$coefficients[3]) / (1 + (AR_OD_MA_T_f_RMD$Time / model_hill_time$coefficients[4])^model_hill_time$coefficients[1]))



### Number of experiments:
AR_OD_MA_T_f_RMD$number_of_experiments <- AR[AR_OD_MA_T_f_RMD$rn, 7]


### Articles reporting quality:
for (ctparameter3 in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter3, 36] <- ARRQSP %>% filter(ARIP1R.StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter3, 1]) %>% select_(23)
}

# Changing column name:
colnames(AR_OD_MA_T_f_RMD)[36] <- paste("Reporting_ratio")


### Year of publication:
for (ctparameter4 in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter4, 37] <- ARRQSP %>% filter(ARIP1R.StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter4, 1]) %>% select_(24)
}


### Journal Impact Factor:
for (ctparameter5 in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter5, 38] <- ARRQSP %>% filter(ARIP1R.StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter5, 1]) %>% select_(25)
}

# Changing the column name:
colnames(AR_OD_MA_T_f_RMD)[38] <- paste("JIF")


### Combined U87 Source:

# As the combined U87 sources were already assessed in AR_updated we can use this dataframe
for (ctparameter6 in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter6, 39] <- AR_updated %>% filter(StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter6, 1]) %>% select_(63) %>% unique()
}
...
for (ctparameter7 in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter7, 40] <- AR_updated %>% filter(StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter7, 1]) %>% select_(65) %>% unique()
}
...
for (ctparameter8 in 1:nrow(AR_OD_MA_T_f_RMD)) {
  AR_OD_MA_T_f_RMD[ctparameter8, 41] <- AR_updated %>% filter(StudyIdStr == AR_OD_MA_T_f_RMD[ctparameter8, 1]) %>% select_(64) %>% unique()
}
...
AR_OD_MA_T_f_RMD$cpc_confluence_num <- AR_updated[AR_OD_MA_T_f_RMD$rn, 68]

# Converting it into numerical values:
AR_OD_MA_T_f_RMD$cpc_confluence_num <- as.numeric(AR_OD_MA_T_f_RMD$cpc_confluence_num)
...
ml_model <- rma.mv(D, Variance_of_D, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist = TRUE, test = "t")
summary(ml_model)

# Confidence interval for the effects estimate (respecting robust-variance-estimation) :
coef_test(ml_model, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(ml_model, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# Prediction interval for the effect:
coef(ml_model) - 1.96 * sqrt(ml_model$sigma2[2])
coef(ml_model) + 1.96 * sqrt(ml_model$sigma2[2])
...
confint.rma.mv(ml_model)

# prediction interval:
predict.rma(ml_model)
...
Glucose <- rma.mv(D, Variance_of_D, mods = ~ factor(glucose), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(Glucose) 

# Confidence interval for the effects estimate (respecting robust-variance-estimation) :
coef_test(Glucose, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(Glucose, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
...
r2_ml(Glucose) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(Glucose)

# Confidence intervals for tau^2 etc.
confint.rma.mv(Glucose)

# Calculating the explained heterogeneity:
(ml_model$sigma2[2] - Glucose$sigma2[2]) / ml_model$sigma2[2] * 100
(ml_model$sigma2[1] - Glucose$sigma2[1]) / ml_model$sigma2[1] * 100
# 12.0 percent of the between-studies-variance gets explained by the glucose level (R^2). Therefor the level of glucose in the culture medium is identified as a driver of irreproducibility.

# Number of effects per phenotype:
table(AR_OD_MA_T_f_RMD$glucose)

# Number of articles per phenotype:
AR_OD_MA_T_f_RMD %>% distinct(StudyIdStr,.keep_all = TRUE) %>% select_(26) %>% table()



### Moderator: U-87 MG source

U87_source <- rma.mv(D, Variance_of_D, mods = ~ factor(combined_U87_source), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(U87_source) 
...
U87_authentification <- rma.mv(D, Variance_of_D, mods = ~ factor(authentification), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(U87_authentification) 
# As this parameter shows no significant result no further parameters are calculated.



### Moderator: U-87 MG age
U87_age <- rma.mv(D, Variance_of_D, mods = ~ U87_age_passages, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(U87_age)
# As this parameter shows no significant result no further parameters are calculated.
...
Cell_conc <- rma.mv(D, Variance_of_D, mods = ~ cell_concentration, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(Cell_conc)
# As this parameter shows no significant result no further parameters are calculated.
...
CPC <- rma.mv(D, Variance_of_D, mods = ~ cpc_confluence_num, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(CPC)
# As this parameter shows no significant result no further parameters are calculated.
...
Mycoplasma <- rma.mv(D, Variance_of_D, mods = ~ factor(mycoplasma), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(Mycoplasma)
# As this parameter shows no significant result no further parameters are calculated.
...
Antibiotics <- rma.mv(D, Variance_of_D, mods = ~ factor(antibiotics), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(Antibiotics)
# As this parameter shows no significant result no further parameters are calculated.
...
FBS_source <- rma.mv(D, Variance_of_D, mods = ~ factor(combined_FBS_source), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(FBS_source)
# As this parameter shows no significant result no further parameters are calculated.
...
Control_Type <- rma.mv(D, Variance_of_D, mods = ~ factor(Control_type_combined), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
...
Rep_ratio <- rma.mv(D, Variance_of_D, mods = ~ Reporting_ratio, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(Rep_ratio)

# Confidence interval for the effects estimate (respecting robust-variance-estimation) :
coef_test(Rep_ratio, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(Rep_ratio, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(Rep_ratio) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(Rep_ratio)

# Confidence intervals for tau^2 etc.
confint.rma.mv(Rep_ratio)



### Moderator: TMZ concentration (4-parameter log-logistic)
LogLog_TMZ_conc <- rma.mv(D, Variance_of_D, mods = ~ loglog4pred, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(LogLog_TMZ_conc)

# Confidence interval for the effects estimate (respecting robust-variance-estimation) :
coef_test(LogLog_TMZ_conc, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(LogLog_TMZ_conc, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(LogLog_TMZ_conc) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(LogLog_TMZ_conc)

# Confidence intervals for tau^2 etc.
confint.rma.mv(LogLog_TMZ_conc)



### Moderator: Treatment duration (4-parameter log-logistic)
LogLog_Time <- rma.mv(D, Variance_of_D, mods = ~ loglog4predtime, random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(LogLog_Time)

# Confidence interval for the effects estimate (respecting robust-variance-estimation) :
coef_test(LogLog_Time, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(LogLog_Time, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(LogLog_Time) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(LogLog_Time)

# Confidence intervals for tau^2 etc.
confint.rma.mv(LogLog_Time)




#### Multivariable meta-analysis with moderators:

### Moderators: TMZ concentration, Treatment duration (both 4-parameter log-logistic)
LogLog_TMZ_conc_Time <- rma.mv(D, Variance_of_D, mods = ~ cbind(loglog4pred,loglog4predtime), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(LogLog_TMZ_conc_Time)

# Confidence interval for the effects estimate (respecting robust-variance-estimation):
coef_test(LogLog_TMZ_conc_Time, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(LogLog_TMZ_conc_Time, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(LogLog_TMZ_conc_Time) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(LogLog_TMZ_conc_Time)

# Confidence intervals for tau^2 etc.
confint.rma.mv(LogLog_TMZ_conc_Time)



### Moderators: TMZ concentration, Treatment duration, Glucose level (first two are 4-parameter log-logistic)
LogLog_TMZ_conc_Time_Glucose <- rma.mv(D, Variance_of_D, mods = ~ cbind(loglog4pred,loglog4predtime,factor(glucose)), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(LogLog_TMZ_conc_Time_Glucose)

# Confidence interval for the effects estimate (respecting robust-variance-estimation):
coef_test(LogLog_TMZ_conc_Time_Glucose, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(LogLog_TMZ_conc_Time_Glucose, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(LogLog_TMZ_conc_Time_Glucose) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(LogLog_TMZ_conc_Time_Glucose)

# Confidence intervals for tau^2 etc.
confint.rma.mv(LogLog_TMZ_conc_Time_Glucose)



### Moderators: TMZ concentration, Treatment duration, Articles reporting quality (first two are 4-parameter log-logistic)
LogLog_TMZ_conc_Time_RQ <- rma.mv(D, Variance_of_D, mods = ~ cbind(loglog4pred,loglog4predtime,Reporting_ratio), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(LogLog_TMZ_conc_Time_RQ)

# Confidence interval for the effects estimate (respecting robust-variance-estimation):
coef_test(LogLog_TMZ_conc_Time_RQ, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(LogLog_TMZ_conc_Time_RQ, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(LogLog_TMZ_conc_Time_RQ) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(LogLog_TMZ_conc_Time_RQ)

# Confidence intervals for tau^2 etc.
confint.rma.mv(LogLog_TMZ_conc_Time_RQ)
...
LogLog_TMZ_conc_Time_Glucose_RQ <- rma.mv(D, Variance_of_D, mods = ~ cbind(loglog4pred,loglog4predtime,factor(glucose),Reporting_ratio), random = list(~ 1 | rn, ~ 1 | StudyIdStr), data = AR_OD_MA_T_f_RMD, method = "REML", tdist=TRUE, test = "t")
summary(LogLog_TMZ_conc_Time_Glucose_RQ)

# Confidence interval for the effects estimate (respecting robust-variance-estimation):
coef_test(LogLog_TMZ_conc_Time_Glucose_RQ, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)
conf_int(LogLog_TMZ_conc_Time_Glucose_RQ, vcov = "CR2", cluster = AR_OD_MA_T_f_RMD$StudyIdStr)

# R2 (using the method described by Nakagawa and Schielzeth, 2012 (https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x))
r2_ml(LogLog_TMZ_conc_Time_Glucose_RQ) # look at marginal ~6.5% explained which is pretty good (this is using the method by https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210x.2012.00261.x)

# I^2
i2_ml(LogLog_TMZ_conc_Time_Glucose_RQ)

# Confidence intervals for tau^2 etc.
confint.rma.mv(LogLog_TMZ_conc_Time_Glucose_RQ)
...
# It seems like the Study "beta-Elemene inhibits proliferation through crosstalk between glia maturation factor beta and extracellular signalÃ¢ÂÂregulated kinase 1/2 and impairs drug resistance to temozolomide in glioblastoma cells" causes problems with the title.
# Fixing this:
...
# Filtering data where the reconciled ROB was different then both ROBs from Joly and Timo
...
## Plotting the risk factors per study distribution
ggplot(aes(x = ROB_r_ov$sum_of_risks_factors), data = ROB_r_ov) +
  geom_histogram(color="black", fill="grey", binwidth = 1) +
  theme_classic() +
  xlab("Number of prevalent risk of bias factors per article") + ylab("Number of articles") +
  scale_x_continuous(limits = c(-0.5,8.5), breaks = seq(0,8,1)) +
  scale_y_continuous(limits = c(0,50), breaks = seq(0,50,10)) +
  ggtitle("Prevalent risk of bias factors per article") +
  theme(axis.ticks.length=unit(.25, "cm")) +
  theme(axis.title = element_text(size = 18)) +
  theme(axis.text = element_text(size = 16)) +
  theme(plot.title = element_text(hjust = 0.5, size = 22, face = "bold")) +
  geom_hline(yintercept=seq(10,50,10), linetype = "dotted", size = 0.7)

# Mean number of risks of bias factors per article:
