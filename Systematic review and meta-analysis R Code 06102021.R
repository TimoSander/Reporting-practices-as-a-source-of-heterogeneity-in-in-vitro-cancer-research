...
AR_OD_MA_T_f$Error[AR_OD_MA_T_f_RMD$StudyIdStr == "e97857cd-aa4e-4af4-9448-6f6973fd6755" & AR_OD_MA_T_f_RMD$Time == 3 & AR_OD_MA_T_f_RMD$tmz_concentration_in_microM_numeric == 50] <- 23.5
AR_OD_MA_T_f$SD[AR_OD_MA_T_f_RMD$StudyIdStr == "e97857cd-aa4e-4af4-9448-6f6973fd6755" & AR_OD_MA_T_f_RMD$Time == 3 & AR_OD_MA_T_f_RMD$tmz_concentration_in_microM_numeric == 50] <- 23.5
AR_OD_MA_T_f$SD.1[AR_OD_MA_T_f_RMD$StudyIdStr == "65b98083-98fc-469f-bd94-4a2074894498" & AR_OD_MA_T_f_RMD$Time == 3] <- 2.191
AR_OD_MA_T_f$SD.1[AR_OD_MA_T_f_RMD$StudyIdStr == "35c0a928-2ad6-4912-919e-46eab215fafb"] <- 2
...
# adding it to AR_OD_MA_T_f_RMD:
escalcnew$cell_concentration <- AR_updated[escalcnew$rn, 62]
...
colnames(escalcnew)[39] <- paste("Reporting_ratio")
...
colnames(escalcnew)[41] <- paste("JIF")
...
for (ctparameter6 in 1:nrow(escalcnew)) {
  escalcnew[ctparameter6, 42] <- AR_updated %>% filter(StudyIdStr == escalcnew[ctparameter6, 1]) %>% select_(63) %>% unique()
}
...
for (ctparameter7 in 1:nrow(escalcnew)) {
  escalcnew[ctparameter7, 43] <- AR_updated %>% filter(StudyIdStr == escalcnew[ctparameter7, 1]) %>% select_(65) %>% unique()
}
...
for (ctparameter8 in 1:nrow(escalcnew)) {
  escalcnew[ctparameter8, 44] <- AR_updated %>% filter(StudyIdStr == escalcnew[ctparameter8, 1]) %>% select_(64) %>% unique()
}
...
# share of unexplained heterogeneity of results across the articles:
LogLog_TMZ_conc_Time_Glucose_RQ_escalc$sigma2[2] / LogLog_TMZ_conc_Time_escalc$sigma2[2]
...
# It seems like the Study "beta-Elemene inhibits proliferation through crosstalk between glia maturation factor beta and extracellular signalÃƒÂ¢Ã‚Â€Ã‚Â‘regulated kinase 1/2 and impairs drug resistance to temozolomide in glioblastoma cells" causes problems with the title.
# Fixing this:
